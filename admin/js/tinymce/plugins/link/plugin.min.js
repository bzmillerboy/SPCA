...
tinymce.PluginManager.add('link', function(editor) {
	...
	function showDialog() {
		...
		function uploadthing() {
			if ($('#hiddenFileUpload').length == 0) {
				$('body').append($('<div id="hiddenFileUpload"></div>', { css: { 'display': 'none' }}));	
			} else {
				$('#hiddenFileUpload').empty();
			}
			
			var uploader = new qq.FileUploaderBasic({
				button: document.getElementById("hiddenFileUpload"),
				...
				onComplete: function(id, fileName, responseJSON){
					...
					$('.mce-link_' + editor.id).val('/upload/documents/content/' + response.filename);
					
					if (!initialText && data.text.length === 0) {
						win.find('#href').parent().parent().parent().find('#text')[0].value(response.filename);
					}
				}
			}); 
			
			$('#hiddenFileUpload input').click();
			return true;
		}
		...
		function updateText() {
			if (!initialText && data.text.length === 0) {
				this.parent().parent().parent().find('#text')[0].value(this.value());
			}
		}
